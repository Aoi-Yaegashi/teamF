...
- 前提条件:
  - DBに商品 `Product(1, "シンプルデスクオーガナイザー", 3500, 3150, "説明A", 10, "/images/desk-organizer.png")` が存在する。
  - セッションに以下のカート情報が存在する:
    - `Cart("id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 3500, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 3500)`
- 手順:
...
- 期待結果:
  1. HTTPステータスコードが `200 OK` であること。
  2. レスポンスボディがJSON形式であり、以下のカート情報を含むこと:
     - `CartItemDTO`: `{"id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 3500, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 4, "subtotal": 4000}` (数量が 1 + 3 = 4 になる)
     - `itemCount`: 4
     - `totalPrice`: 4000
...
- 前提条件:
  - DBに商品 `Product(1, "シンプルデスクオーガナイザー", 3500, 900, "説明A", 10, "//images/desk-organizer.png")`, `Product(2, "アロマディフューザー（ウッド）", 2000, 1800, "説明B", 5, "//images/aroma-diffuser.png")` が存在する。
  - セッションに以下のカート情報が存在する:
    - `Cart("id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 3500, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 3500)`
- 手順:
...
- 期待結果:
  1. HTTPステータスコードが `200 OK` であること。
  2. レスポンスボディがJSON形式であり、以下のカート情報を含むこと:
     - `CartItemDTO`: `{"id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 3500, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 3500}, {"id": "1", "productId": 2, "name": "アロマディフューザー（ウッド）", "price": 2000, "imageUrl": "(/images/aroma-diffuser.png画像URL)", "quantity": 1, "subtotal": 2000}`
     - `itemCount`: 2 (1 + 1)
     - `totalPrice`: 3000 (3500 + 2000)
  3. セッションの `cart` 属性の内容がレスポンスボディと同じカート情報に更新されていること。
...
- 期待結果:
  1. HTTPステータスコードが `200 OK` であること。
  2. レスポンスボディがJSON形式であり、以下のカート情報を含むこと:
     - `CartItemDTO`: `{"id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 3500, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 3500}`
  3. セッションの `cart` 属性の内容がレスポンスボディと同じカート情報に更新されていること。
...
- 前提条件:
  - DBに商品 `Product(1, "シンプルデスクオーガナイザー", 3500, 900, "説明A", 10, "//images/desk-organizer.png")`, `Product(2, "アロマディフューザー（ウッド）", 2000, 1800, "説明B", 5, "//images/aroma-diffuser.png")` が存在する。
  - セッションに以下のカート情報が存在する:
    - `Cart({"id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 3500, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 3500}, {"id": "1", "productId": 2, "name": "シンプルデスクオーガナイザー", "price": 2000, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 2000})`
- 手順:
...
- 期待結果:
  1. HTTPステータスコードが `200 OK` であること。
  2. レスポンスボディがJSON形式であり、以下のカート情報を含むこと:
     - `CartItemDTO`: `{"id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 2000, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 2000}` (商品2が削除されている)
     - `itemCount`: 1
     - `totalPrice`: 2000
  3. セッションの `cart` 属性の内容がレスポンスボディと同じカート情報に更新されていること。
...
- 前提条件
  - DBに商品 `Product(1, "シンプルデスクオーガナイザー", 3500, 900, "説明A", 10, "//images/desk-organizer.png")`, `Product(2, "アロマディフューザー（ウッド）", 2000, 1800, "説明B", 5, "//images/aroma-diffuser.png")` が存在する。
  - セッションに以下のカート情報が存在する：
    - `CartItemDTO`: `{"id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 3500, "imageUrl": "//images/desk-organizer.png", "quantity": 2, "subtotal": 2000}`, `{"id": "2", "productId": 2, "name": "アロマディフューザー（ウッド）", "price": 2000, "imageUrl": "//images/aroma-diffuser.png", "quantity": 1, "subtotal": 2000}`
- 手順
...
- 前提条件:
  - DBに商品 `Product(1, "シンプルデスクオーガナイザー", 3500, 900, "説明A", 10, "//images/desk-organizer.png")` が存在する。
  - セッションにカート情報 `Cart({"id": "1", "productId": 1, "name": "シンプルデスクオーガナイザー", "price": 3500, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 3500}, {"id": "1", "productId": 2, "name": "シンプルデスクオーガナイザー", "price": 2000, "imageUrl": "(/images/desk-organizer.pngURL)", "quantity": 1, "subtotal": 2000})` が存在する。
- 手順:
