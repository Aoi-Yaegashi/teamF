...
- 前提条件:
  - DBに商品 `Product(id=1, productName="商品A", unitPrice=500, quantity=10)`, `Product(id=2, productName="商品B", unitPrice=1000, quantity=5)` が存在する。
  - セッションにカート `Cart(items={"1": CartItem(..., quantity=2, subtotal=1000), "2": CartItem(..., quantity=1, subtotal=1000)}, itemCount=3, totalPrice=2000)` が存在する。
- 手順:
