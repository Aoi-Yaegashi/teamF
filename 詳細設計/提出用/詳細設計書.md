...
各主要機能について、機能詳細をシーケンス図にて図示する。
  ...
  ### 3.2.1 商品閲覧・購入機能（購入者向け）
   ...
   #### 3.2.1.1 商品一覧・カテゴリ別の商品閲覧機能(MUST) 
   <div class="mermaid">
   sequenceDiagram
    participant User as ユーザー
...
sequenceDiagram
   ...
   #### 3.2.1.3 商品のカート追加／削除／数量変更機能（MUST）
   **商品のカート追加**
   <div class="mermaid">
   sequenceDiagram
    participant User as ユーザー
...
**2. 削除** (上記PUTと同様の流れ/DELETE /api/cartで実行)
   ...
   #### 3.2.1.4 カート内商品の一覧、数量、小計、合計金額の表示機能（MUST）
<div class="mermaid">
sequenceDiagram
    participant User as ユーザー
    participant Browser as フロントエンド
...
sequenceDiagram
   ...
   #### 3.2.1.6 注文完了後の確認メール自動送信機能(Should)
   <div class="mermaid">
   sequenceDiagram
    participant User as ユーザー
    participant Browser as フロントエンド
...
| 5   | カートページ表示                       | F04    | POST /api/cart                  | GET      | セッションID or ユーザーID                                                                                       | 現在のカート情報                                                                                 | カート一覧を表示                     |
...
| 8   | 購入手続きページ表示                   | F05    | POST /api/cart/checkout         | POST     | セッションID or ユーザーID                                                                                       | 購入予定カート情報                                                                               | 購入フォームを表示                     |
