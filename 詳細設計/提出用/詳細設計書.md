...
<<<<<<< HEAD
   ...
   sequenceDiagram
    ...
    participant TPC as ProductController
    participant TPS as ProductService
    ...
    %% フロントエンドがAPI呼び出し
    FE->>TPC: GET /api/product

    %% コントローラがサービスに処理を依頼
    TPC->>TPS: getNewProductsData()
...
>>>>>>> c701c0a4d878665a65c66999082773ddbcd087f5
    ...
    %% コントローラがサービスに処理を依頼
<<<<<<< HEAD
    PLC->>PLS: getProductList(keyword)

    %% サービスが商品一覧を取得
    PLS->>ProdRepo: findProducts(keyword)
=======
