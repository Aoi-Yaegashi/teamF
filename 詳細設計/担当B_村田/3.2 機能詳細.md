...
sequenceDiagram
    ...
    CartS->>CartItemRepo:getSubtotal()
    CartItemRepo-->>CartS:小計金額情報

    CartS->>CartRepo: getTotalPrice()
    CartRepo-->>CartS: 合計金額情報
    CartS->>CartRepo: getItemCount()
    CartRepo-->>CartS: 数量情報
...
sequenceDiagram
    ...
    %% 4. サービスが商品情報を取得（在庫チェックのため）
    CartS->>ProdRepo: getProductById(id)
    ProdRepo-->>CartS: 商品情報（在庫数含む）
...
sequenceDiagram
    ...
    %% 3. カート情報を取得
    OrderC->>CartS: getCart(session)
    CartS->>CartRepo: findById(id)
    CartRepo-->>CartS:カート情報
    CartS-->>OrderC: 購入予定カート情報

    %% 4. 注文作成処理
    OrderC->>OrderS: createOrder(dto)
    OrderS->>OrderRepo: Order save (Order order)
    OrderRepo-->>OrderS:注文作成完了
    OrderS-->>OrderC: 注文作成完了

    %% 5. 注文情報を取得
    OrderC->>OrderS: getOrderById(id)
